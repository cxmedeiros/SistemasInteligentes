...
class World {
  ...
  run() {
    ...
    for (let i = this.bloops.length - 1; i >= 0; i--) {
      ...
      let nearestBloop = [...this.bloops].sort((first,second)=>p5.Vector.dist(b.position, first.position) - p5.Vector.dist(b.position, second.position))[1] //bloco mais próximo para ocorrer a reprodução
      let child = b.reproduce(nearestBloop);
